    ...
    if (task.task_id_external && task.status !== "pending") {
      ...
      try {
        ...
        if (externalTask.error) {
          ...
          const errorMsg = externalTask.error?.message || "Task failed";
        ...
        if (externalTask.status === "succeeded") {
          ...
          if (task.queued_at && updates.started_at) {
            updates.queue_duration = Math.round(
              (new Date(updates.started_at as string).getTime() - new Date(task.queued_at as string).getTime()) / 1000
            );
          }
          ...
          if (task.started_at && updates.completed_at) {
            updates.generation_duration = Math.round(
              (new Date(updates.completed_at as string).getTime() - new Date(task.started_at as string).getTime()) / 1000
            );
            ...
            const queuedTime = new Date(task.queued_at as string).getTime();
        ...
        return NextResponse.json({
          ...
          result: updates.result ? JSON.parse(updates.result as string) : task.result,
...
async function uploadVideoToTos(taskId: string, videoUrl: string, projectId: string, userConfig?: TosConfig | null): Promise<void> {
  ...
  try {
    ...
    const db = getDb();
    db.prepare("UPDATE tasks SET permanent_video_url = ?, video_storage_key = ? WHERE id = ?")
      .run(result.url, result.key, taskId);
